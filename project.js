...
// //     function animateYearStep() {
      
// //       if (!window.timelapseState?.isPlaying) return;

// //       const yearList = window.timelapseState.yearsArray;
// //       const index = window.timelapseState.currentYearIndex;

// //       if (index >= yearList.length) {
// //         window.timelapseState.currentYearIndex = 0; // Replay from beginning
// //         animateYearStep();
// //         return;
// //       }
...
// //       const slider = document.querySelector("#year-slider-container .yearSlider");
...
// //       window.timelapseState.currentYearIndex++;
// //       window.timelapseState.animationId = setTimeout(
// //         animateYearStep,
// //         window.timelapseState.duration / yearList.length
// //       );
// //     }
...
function createYearSlider(yearID, yearsVar, minY, maxY) {
  const yearSlider = document.querySelector(yearID);
  ...
  function updateYearByIndex(index) {
    const year = yearsVar[index];
    userYear = year;
    yearSlider.noUiSlider.set(year);
    updateEventHandler([year], 0);
  }
  ...
  function startTimeLapse() {
    ...
    animationId = setInterval(() => {
      if (currentYearIndex < yearsVar.length) {
        updateYearByIndex(currentYearIndex);
        currentYearIndex++;
      } else {
        stopTimeLapse();
        currentYearIndex = 0;
  ...
  function resumeTimeLapse() {
    if (!isAnimating && currentYearIndex < yearsVar.length) {
      startTimeLapse();
    }
  }

  yearSlider.noUiSlider.on('start', () => {
    if (isAnimating) stopTimeLapse();
  });
